...
桌面版入口是 `desktop_app.py`，使用配套 Chrome 扩展直接读取 WhatsApp Web 页面结构，已经取消屏幕坐标和鼠标拖选：
...
2. 开启“开发者模式”。
3. 点击“加载已解压的扩展程序”。
4. Windows 解压 `dist/windows/Chrome-Extension.zip` 后选择该文件夹；macOS 使用 DMG 内的 `Chrome-Extension` 文件夹。
5. 桌面 App 与扩展选项的端口都保持 `8765`。
6. 桌面 App 点击“启动扩展服务”，然后刷新 WhatsApp Web。

扩展启动时会忽略当前页面已有历史消息，只处理扩展就绪后新收到的消息。
